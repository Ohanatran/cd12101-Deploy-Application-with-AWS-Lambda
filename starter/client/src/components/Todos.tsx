...
import { convertDynamoDBResponseToJSON } from '../helpers/convertJson'
...
export class Todos extends React.PureComponent<TodosProps, TodosState> {
  ...
  onTodoCreate = async (event: React.ChangeEvent<HTMLButtonElement>) => {
    ...
    try {
      ...
      const newValue = convertDynamoDBResponseToJSON([newTodo]).items[0];
      this.setState({
      ...
      Swal.fire({
        position: 'top-end',
        icon: 'success',
        title: `${newValue.name} has been create`,
  ...
  async componentDidMount() {
    try {
      const todosResponse = await getTodos(this.props.auth.getIdToken())
      this.setState({
        todos: convertDynamoDBResponseToJSON(todosResponse).items,
        loadingTodos: false
      })
    } catch (e) {
